...
logger = logging.getLogger("re-architect.binary_loader")
    ...
    def _check_tools(self) -> None:
        ...
        logger.debug(f"Available tools: {[k for k, v in self.available_tools.items() if v]}")iles.
"""
...
class BinaryLoader:
    ...
    def _lief_is_library(self, binary) -> bool:
        ...
        if binary.format == lief.Binary.FORMATS.ELF:
            return hasattr(binary, 'header') and binary.header.file_type == lief.ELF.E_TYPE.DYNAMIC
        elif binary.format == lief.Binary.FORMATS.PE:
            ...
            return hasattr(binary, 'header') and binary.header.file_type == lief.MachO.FILE_TYPES.DYLIB
        return False
